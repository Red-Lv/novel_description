...
class LCS(object):
    ...
    def cal_cs_threshold(self):
        ...
        return math.floor((1 - (math.tanh(math.log(len(seq_list) / 100.0)) + 1) / 2.0 * 0.95) * len(seq_list))
    ...
    def check_cs_existence(self, k):
        ...
        for i, j in potential_sa:
            ...
            if len(str_cover_dict) >= threshold:
                ...
                if cs.find(self.sep) == -1 and cs.find(u'\u0003') != -1:
                    cs_dict[cs] = (cs, len(str_cover_dict))

        return cs_dict.values()
    ...
    def gen_lcs(self):
        ...
        for i in range(right, 1, -1):
            ...
            i_cs_valid_list = []
            for item, _cnt in i_cs_list:

                valid = True
                for cs, cnt in cs_list:
                    #if cs.find(item) != -1 and cnt >= _cnt:
                    if cs.find(item) != -1:
                        valid = False
                        break
                
                if valid: 
                    i_cs_valid_list.append((item, _cnt))
            ...
            print i
            for item, _cnt in i_cs_valid_list:
                print u'\u001a'.join([item]).encode('GBK', 'ignore')
